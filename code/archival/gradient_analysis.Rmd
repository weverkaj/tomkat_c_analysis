...
zz = fun_table_soil %>% 
  rownames_to_column() %>% 
  left_join(com_table_fun_all %>% rownames_to_column(), by = "rowname") %>% 
  clean_names() %>% 
  mutate(pg_change = perennial_grass_x - perennial_grass_y)
```
...
```{r}
veg_change_NMDS = metaMDS(veg_change_com, distance = "euclidian", k = 2, maxit = 999, trymax = 500, wascores = TRUE, autotransform = T)
...
p_change= tibble(p_change = fun_table21$perennial_grass - fun_table_soil_future$perennial_grass)

x = fun_table_soil_future %>% bind_cols(p_change) %>% tibble()
...
```{r}
ggplot(x, aes(x = p_change, y = surface)) +
  geom_point() +
  xlab("Perennial Grass Change 2018-21") +
  ylab("SOC Change 0-10 cm") +
  geom_label_repel(aes(label = rowname), min.segment.length = 0)


ggplot(x, aes(x = p_change, y = depth)) +
  geom_point() +
  xlab("Perennial Grass Change 2018-21") +
  ylab("SOC Change 10-40 cm") +
  geom_label_repel(aes(label = rowname), min.segment.length = 0)
