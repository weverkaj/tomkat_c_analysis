...
```{r}
ggplot(microbe_diversity_soil, aes(x = diversity, y = change18, color = depth_cm)) +
  geom_point() +
  facet_wrap(. ~ depth_cm) +
  geom_smooth(method = "lm")

ggplot(microbe_diversity_soil, aes(x = richness, y = change18, color = depth_cm)) +
  geom_point() +
  facet_wrap(. ~ depth_cm) +
  geom_smooth(method = "lm")
```
...
napu_mod = lm(`2018_40` ~ DACA3, data = veg_table_soil)
summary(napu_mod)
...
pg_mod = lm(x2018_10 ~ annual_forb, data = fun_table_soil)
summary(pg_mod)

pg_mod40 = lm(x2018_40 ~ annual_forb, data = fun_table_soil)
summary(pg_mod40)
...
```{r}
(p_a_change = ggplot(fun_table_soil_future, aes(x = p_a_grass, y = surface)) +
  geom_point() +
  geom_smooth(method = "lm", color = "black") +
  theme_bw() +
  theme(axis.title = element_text(size = 14)) +
...
pg_c = lm(surface ~ p_a_change, data = fun_table_soil_change)
summary(pg_c)
...
```
```{r}
ggplot(fun_table_soil_change, aes(x = p_a_change, y = surface)) + 
  geom_point()
```
