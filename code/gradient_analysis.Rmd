...
```{r}
ggplot(microbe_diversity_soil, aes(x = diversity, y = change21)) +
  geom_point() +
  facet_wrap(. ~ depth_cm) +
...
```{r}
divmod_surf = lm(`2018` ~ diversity, data = microbe_diversity_soil)
summary(divmod_surf)

divmod_depth = lm(`2018` ~ diversity, data = microbe_diversity_soil40)
summary(divmod_depth)

divmod_depth_change = lm(change18 ~ diversity, data = microbe_diversity_soil40)
summary(divmod_depth_change)

divmod_surf_change = lm(change18 ~ diversity, data = microbe_diversity_soil10)
summary(divmod_surf_change)


divmod_surf_future = lm(change21 ~ diversity, data = microbe_diversity_soil10)
summary(divmod_surf_future)

divmod_depth_future = lm(change21 ~ diversity, data = microbe_diversity_soil0)
...
diversity_soil = diversity(com_table, index = "shannon") %>% 
  as.data.frame() %>% 
  mutate(diversity = divs$.) %>% 
  select(diversity) %>% 
  rownames_to_column() %>% 
  left_join(soil_veg %>% rownames_to_column(), by = "rowname") %>% 
  left_join(soil_veg_future, by = c("rowname" = "point_id")) %>% 
...
div_surf_mod = lm(`2018_10` ~ diversity, data = diversity_soil)
summary(div_surf_mod)

div_depth_mod = lm(`2018_40` ~ diversity, data = diversity_soil)
summary(div_depth_mod)

div_surf_change_mod = lm(change18_10 ~ diversity, data = diversity_soil) #significant
summary(div_surf_change_mod)

div_depth_change_mod = lm(change18_40 ~ diversity, data = diversity_soil)
summary(div_depth_change_mod)

div_surf_future_mod = lm(surface ~ diversity, data = diversity_soil)
summary(div_surf_future_mod)

div_depth_future_mod = lm(depth ~ diversity, data = diversity_soil)
...
```{r}
ggplot(diversity_soil, aes(x = diversity, y = change18_10)) +
  geom_point() +
  geom_smooth(method = "lm")
```
